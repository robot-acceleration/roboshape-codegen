...
class rbdReference:
    ...
    def rnea_grad_bpass_dqd(self, q, df_dqd):
        print("Printing intermediate bpass dqd")
        ...
        for ind in range(n-1,-1,-1):
            ...
            if parent_ind != -1:
                ...
                if parent_ind == 0:
                    tmp = np.matmul(np.transpose(Xmat),df_dqd[:,:,ind])
                    print("curr_ind" + " " + str(ind))
                    print(np.matmul(np.transpose(S),tmp))
                    print("parent_ind" + " " + str(parent_ind))
                    print(np.matmul(np.transpose(S),df_dqd[:,:,parent_ind]))

        print("End intermediate bpass dqd")
        return dc_dqd
