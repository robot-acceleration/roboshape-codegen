...
fm = FileManager("GradientPipelineBprocMatmulFirstInputUnrolling.bsv")
...
for curr_PE in range(num_PEs):
    for block_c in range(block_size):
        for block_r in range(block_size):
            minv_r = lh_tile_row_start_sched[0][curr_PE] + block_r
            minv_c = lh_tile_col_start_sched[0][curr_PE] + block_c
            # slotting matmul output vector into right place in the final big matrix
            bw.writeLine("        bproc.minv_block_in_R{block_r}_C{block_c}_dqdPE{curr_PE}(minv[{minv_r}][{minv_c}]);".format(
    ...
    for block_r in range(block_size):
        dtau_r = rh_tile_row_start_sched[0][curr_PE] + block_r
        dtau_c = rh_tile_col_start_sched[0][curr_PE] + rh_tile_col_sched[0][curr_PE]
        if rh_tile_col_sched[0][curr_PE] < block_size:
            bw.writeLine("        bproc.dtau_vec_in_R{block_r}_dqdPE{curr_PE}(new_dtaudq[{dtau_r}][{dtau_c}]);".format(
                block_r=block_r+1, curr_PE=curr_PE+1,
                dtau_r=dtau_r, dtau_c=dtau_c))
        else:
            bw.writeLine("        bproc.dtau_vec_in_R{block_r}_dqdPE{curr_PE}(0);".format(
                block_r=block_r+1, curr_PE=curr_PE+1
            ))
    bw.writeLine("")
